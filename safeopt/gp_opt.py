...
class GaussianProcessOptimization(object):
    ...
    def __init__(self, gp, fmin, beta=2, num_contexts=0, threshold=0,
        ...
        print('This is local package in rbc-parameter-tuning')
    ...
    def plot(self, n_samples, axis=None, figure=None, plot_3d=False,figsize=(5,5),
             **kwargs):
        """
        ...
        print(self.gp.input_dim,self.gp.kern.input_dim,self.num_contexts)
        ...
        if self.gp.input_dim - self.num_contexts == 1:
            ...
            if plot_3d:
                ...
                for gp, fmin in zip(self.gps, self.fmin):
                    ...
                    #                 figure=figure,
                    #                 axis=axis,**kwargs)
                    plot_contour_gp(gp, inputs, figure=figure, axis=axis,n_samples=n_samples,fmin=fmin, **kwargs)
...
class SafeOpt(GaussianProcessOptimization):
    ...
    def context_fixed_inputs(self):
        ...
        if nc > 0:
            ...
            print('context_fixed_inputs: ',list(zip(range(n, n - nc, -1), contexts)))
    ...
    def get_maximum(self, context=None,beta_estimate=None):
        """
        ...
        if beta_estimate is not None:
            beta_=self.beta
            self.beta=lambda t: beta_estimate
...
if __name__=='__main__':
    ...
    with open('../optimization_sim/CSBO_ucb_b2_winter.p', 'rb') as file:
        model = pickle.load(file)
    bounds = [(0., 4.),(-1,1)]
    parameter_set = linearly_spaced_combinations(bounds, 100)       
    opt = SafeOpt(model,parameter_set=parameter_set,fmin=0,num_contexts=1)
    opt.context=[[0.5]]
    opt.plot(100,levels=[-1,0,1])
    opt.context=[[0]]
    opt.plot(100,levels=[-1,0,1])
    plt.show()