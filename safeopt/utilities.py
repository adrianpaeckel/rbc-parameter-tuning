...
def plot_2d_gp(gp, inputs, predictions=None, figure=None, axis=None,
               fixed_inputs=None, beta=3, fmin=None, figsize=(5,5),**kwargs):
        """
...
def plot_3d_gp(gp, inputs, predictions=None, figure=None, axis=None,
               fixed_inputs=None, beta=3,figsize=(5,5), **kwargs):
        """
...
def plot_contour_gp(gp, inputs, predictions=None, figure=None, axis=None,
                    colorbar=True,fixed_inputs=None,n_samples=[50,50],fmin=None,figsize=(5,5), **kwargs):
        """
        Plot a 3D gp with uncertainty.
        ...
        """
        bounds=[(inputs[0,0],inputs[-1,0]),(inputs[0,1],inputs[-1,1])]
        if fixed_inputs is None:
            if gp.kern.input_dim > 2:
                raise NotImplementedError('This only works for 2D inputs')
            fixed_inputs = []
        ...
        if predictions is None:
            ...
            mean, var = predictions
        c_bar = None
        # print(inputs,fixed_inputs)
        # print(inputs.reshape(n_samples))
        mesh=np.meshgrid(np.linspace(bounds[0][0],bounds[0][1],n_samples[0]),
                                     np.linspace(bounds[1][0],bounds[1][1],n_samples[1]),indexing='xy')
        # print(mesh)
        # print(mean.squeeze().reshape(n_samples).shape)
        if not np.all(mean == mean[0]):
            # Need to squeeze the added dimensions caused by fixed inputs
            c = axis.contour(mesh[0],mesh[1],mean.squeeze().reshape(n_samples),**kwargs)
            if fmin!=-np.inf:
                axis.contourf(mesh[0],mesh[1],mean.squeeze().reshape(n_samples),levels=[-np.inf,fmin,np.inf],colors=['grey','white'],linestyles='-',alpha=0.5)
            if colorbar:
                c_bar = plt.colorbar(c)
        else:
            c = None

        data = axis.plot(gp.X[:-1, 0], gp.X[:-1, 1], 'xb')
        axis.plot(gp.X[-1, 0], gp.X[-1,1], 'ob')
...
def plot_contour_gp_2(gp, inputs, predictions=None, figure=None, axis=None,
        ...
        if axis is None:
            if figure is None:
                _kwargs=kwargs.copy()
                _kwargs.pop('fixed_inputs',3)
                figure = plt.figure(**_kwargs)
                axis = figure.gca()
            else:
                axis = figure.gca()
